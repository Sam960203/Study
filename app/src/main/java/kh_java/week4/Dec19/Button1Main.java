package kh_java.week4.Dec19;// 1219 1,2교시

import java.awt.event.ActionEvent;
//java.lang패키지가 아닌것은 모두 import ykdyd
...
class Button1 extends JFrame {
  // 선언부
  Button1Event bevent = new Button1Event(this);// 상속,추상클래스 중심,인터페이스 중심 코딩전개
  // 게으른 인스턴스화
  // 북쪽배치
  JButton jbtn_ins = null;// 선언만 했다. 생성자 호출안됨
  // 이른 인스턴스화
  // 남쪽배치
  JButton jbtn_upd = new JButton("수    정"); // 선언 및 생성 동시에 생성자 호출까지 일어남
  // 생성자

  Button1() {
    System.out.println("Button 1 디폴트 생성자 호출" + jbtn_ins);
  ...
  public void initDisplay() {
    ...
    // 동쪽 배치
    JButton jbtn_del = new JButton("삭  제");
...
class Button1Event implements ActionListener {
  ...
  public Button1Event(Button1 button1) {
    this.button1 = button1;// 이거 모르면 심각하다.
  }
