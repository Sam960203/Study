...
//선언과 생성을 분리하는 코딩전개 
//자바는 단일 상속가능 두개 안됨 
//자바는 단일 상속의 단점을 보완하기 위해 인터페이스는 다중으로 처리 가능함(구현체클래스)
public class TalkServer extends JFrame implements Runnable, ActionListener {
  // 선언부
  // 클라이언트 촉에서 new Socket 하면 그 소켓정보를 받아서 쓰레드로 넘김
  ...
  // 생성자
  public TalkServer() {
    // initDisplay();// 시점문제 - 스케줄링 여기서 하면 망함 그럼 어디서 해 메인 ㄱ ㄱ
  }
  ...
  public static void main(String[] args) {
    ...
    th.start();// run 호출함 지연발생함 클라이언트가 접속할때까지 기다림...
  }
  ...
  public void run() {
    ...
    globalList = new Vector<>();// 멀티스레드안전에서 Arraylist대신 사용함
    boolean isStop = false;
    try {
      server = new ServerSocket(3000);// 서버포트번호 설정하기
      jta_log.append("Loading.........\n");// 대기 탐 -멈춤 - wait
      while (!isStop) {
